def register(roundl):
	...
	while f==0:
		...
		dot=mail.find('.',at)
		if '@' not in mail:
		    print("Incorrect e-mail. Enter correct e-mail format.")
		elif l.count('@')>=2:
			print("Incorrect e-mail. Enter correct e-mail format.")
		elif mail[at+1]=='.':
		    ...
		    print("Incorrect e-mail. Enter correct e-mail format.")
		elif len(mail[:at])<3 or mail[0] in spl or mail[0].isdigit() or [True for i in mail[:at] if i in spl]:
		    print("Incorrect e-mail. Enter correct e-mail format.")
		elif mail[-4:]!='.com' and mail[-3:]!='.in': 
		    print("Incorrect e-mail. Enter correct e-mail format.")
		else:
		...
		if f==0:
			...
			if fp.lower()=='try again':
					...
					if mail in word:
						...
						if opt.lower()=='retrieve password':
							...
							while np==0:
								...
								newpwd=input()
								if len(newpwd)<=5 or len(newpwd)>15:
									print('Not satisfied condition. Enter password in correct format')
								else:
									for i in range(len(newpwd)):
										if newpwd[i].isupper():
											u=1
										elif newpwd[i].islower():
											lo=1
										elif newpwd[i] in spl:
											s=1
										elif newpwd[i].isdigit():
											d=1
										if u==1 and lo==1 and s==1 and d==1:
											np=1
											break
				...
				if m==0:
					...
					register(roundl)
					exit()
			elif fp.lower()=='register':
				roundl+=1
				register(roundl)
				exit()

print('Enter \'register\' or \'login\':',end=' ')
entry=input()
roundl=0
